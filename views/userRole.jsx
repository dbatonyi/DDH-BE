          ...
          <div className="dashboard-container__main">
            {props.systemMessage ? <div className='system-message'>{props.systemMessage}</div> : null}
                  <div className="user-role-container">
                      <div>{username} current permission: { role }</div>
                      <div>Change permission</div>
                      <form className="user-role-container__form" id="userRole" name="userRole" encType="multipart/form-data" method="post" action={`/update-user-role${id}`}>
